...
struct Zoltan_Comm_Obj {    /* data for mapping between decompositions */
    ...
    uint64_t *starts_to;    /* where in item lists each send starts */
    uint64_t *starts_from;    /* where in item lists each recv starts */

    /* Following arrays used is send/recv data not packed contiguously */
    uint64_t *indices_to;       /* indices of items I send in my msgs */
    /* ordered consistent with lengths_to */
    uint64_t *indices_from;     /* indices for where to put arriving data */
    ...
    uint64_t *starts_to_ptr;    /* where in dense array sends starts */
    uint64_t *starts_from_ptr;    /* where in dense each recv starts */

    /* Following used is send/recv data not packed contiguously & items vary */
    uint64_t *indices_to_ptr;   /* where to find items I send in my msgs */
    /* ordered consistent with lengths_to */
    uint64_t *indices_from_ptr; /* where to find items I recv */
    /* ordered consistent with lengths_from */
